...
class CarLoanWin(Frame):
    ...
    def Main_CarLoan(self):
        self.master.title("Car Loan")
        ...
        self.BGMainPage = ImageTk.PhotoImage(Image.open("assets/Slide17.jpg"))
        ...
        self.Confirm = Button(self, bd = 0, state = DISABLED, height = 74, width = 330, command = self.End_CarLoan)
        self.Confirm.config(image = self.ButtonConfirm)
        self.Confirm.place(x = 548, y = 595)

    def MonthlyPayment(self):
        self.CarLoan_Payment = 'Monthly'
        self.Confirm.config(state="normal")

    def QuarterlyPayment(self):
        self.CarLoan_Payment = 'Quarterly'
        self.Confirm.config(state="normal")

    def End_CarLoan(self):
        global root10
        root10 = Toplevel(self.master)
        root10.title("Car Loan Confirmation")
        root10.geometry("1366x768")
        root10.state('zoomed')
        a1 = CarLoanWin_End(root10)

class CarLoanWin_End(Frame):

    def __init__(self, master):
        Frame.__init__(self, master)
        self.End_CarLoanWindow()

    def End_CarLoanWindow(self):
        self.master.title("Car Loan Confirmation")
        self.pack(fill = BOTH, expand = 1)

        self.ButtonMainMenu = ImageTk.PhotoImage(Image.open("assets/Main Menu.png"))
        self.BGCarLoanEnd = ImageTk.PhotoImage(Image.open("assets/Slide18.jpg"))
        tk.Label(self,image = self.BGCarLoanEnd).pack()
